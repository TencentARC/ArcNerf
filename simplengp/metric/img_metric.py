...
from simplengp.datasets.transform.augmentation import srgb_to_linear
...
@METRIC_REGISTRY.register()
class sRGBtoLinearPSNR(PSNR):
    """PSNR for image and gt, where original rgb is in sRGB space """

    def __init__(self, cfgs=None):
        super(sRGBtoLinearPSNR, self).__init__(cfgs)

    def forward(self, data, output):
        """Change into linear space first"""
        data['img'] = srgb_to_linear(data['img'])
        output[self.key] = srgb_to_linear(output[self.key])

        return super().forward(data, output)
