...
from simplengp.geometry.ray import closest_point_to_rays
from simplengp.geometry.poses import center_poses, average_poses
...
class Base3dDataset(BaseDataset):
    ...
    def precache_ray(self):
        ...
        if self.ray_bundles is None:
            ...
            for i in range(self.n_imgs):
                self.ray_bundles.append(self.cameras[i].get_rays(wh_order=False, center_pixel=self.center_pixel))
    ...
    def __getitem__(self, idx):
        ...
        img = self.images[idx].reshape(-1, 3)  # (hw, 3)print(self.get_bitfield_count()[1])
        if len(self.masks) == 0:  # Mask for all images as one
            mask = np.ones((img.shape[0],), dtype=img.dtype)
        else:
            mask = self.masks[idx].reshape(-1)  # (hw)
        ...
        if self.precache:
            ...
            ray_bundle = self.cameras[idx].get_rays(wh_order=False, center_pixel=self.center_pixel)
