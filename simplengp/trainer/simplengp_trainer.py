...
from simplengp.datasets import get_dataset, get_model_feed_in, POTENTIAL_KEYS
from simplengp.datasets.transform.augmentation import get_transforms
from simplengp.eval.eval_func import run_eval
from simplengp.geometry.point_cloud import save_point_cloud
from simplengp.loss import build_loss
from simplengp.metric import build_metric
from simplengp.models import build_model
from simplengp.trainer.ema import EMA
from simplengp.visual.render_img import render_progress_imgs, write_progress_imgs
...
class SimplengpTrainer(BasicTrainer):
    ...
    def __init__(self, cfgs):
        ...
        # call parent init
        super(SimplengpTrainer, self).__init__(cfgs)
    ...
    def train_step_writer(
        ...
        # write occ ratio
        if epoch % self.cfgs.progress.epoch_loss == 0 and step % self.cfgs.progress.iter_loss == 0:
            self.logger.add_log('Remain voxel ratio - {:.2f}%...'.format(
                self.model.get_dense_grid().get_bitfield_count()[1] * 100.0)
            )
        ...
        if self.cfgs.progress.save_progress and self.cfgs.progress.local_progress:
            if epoch % self.cfgs.progress.epoch_save_progress == 0:
                occ_pc = torch_to_np(self.model.get_dense_grid().get_occ_pc())
    ...
    def step_optimize(self, epoch, step, feed_in):
        ...
        output = self.model(feed_in, cur_epoch=epoch)
        ...
        for epoch in range(self.cfgs.progress.start_epoch, self.cfgs.progress.epoch):
            ...
            if self.data['val'] is not None and self.cfgs.progress.epoch_val > 0:
                if epoch > 0 and epoch % self.cfgs.progress.epoch_val == 0:
                    ...
                    self.model.reset_measure()  # reset the measure for dynamic batchsize
