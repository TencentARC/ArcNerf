...
class Pipeline(object):
    ...
    def step_dynamic_bs(self, logger):
        ...
        if valid_key_in_cfgs(self.scheduler_cfg, 'dynamic_batch_size') \
            ...
            logger.add_log(
                'Dynamically adjust training batch size every {} epoches...max bs allow {}' .format(update_epoch,
                                                                                                    max_batch_size)
            )
    ...
    def fetch_step_update_dynamic_bs(self, epoch, model):
        ...
        if self.get_info('dynamic_batch_size') > 0:
            ...
            if epoch % update_epoch == update_epoch - 1 and epoch > 500:  # fix warmup
                dynamic_factor = model.get_dynamic_factor()
